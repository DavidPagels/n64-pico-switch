...
void Controller::initPio( InitParams *initParams )
{
    ...
    pio_sm_config tmpConfig = controller_program_get_default_config( initParams->offset );
    initParams->c = &tmpConfig;
    controller_program_init(
        initParams->pio, initParams->sm, initParams->offset, initParams->pin, initParams->c
    );
    ...
    transfer(
        initParams->pio, initParams->sm, initRequest, sizeof( initRequest ), initResponse,
        sizeof( initResponse )
    );
    initParams->controllerType = (ControllerType)initResponse[0];
}
void Controller::transfer(
    uint8_t *request, uint8_t requestLength, uint8_t *response, uint8_t responseLength
)
{
...
void Controller::transfer(
    PIO pio, uint sm, uint8_t *request, uint8_t requestLength, uint8_t *response,
...
void Controller::sendRequest( PIO pio, uint sm, uint8_t *request, uint8_t requestLength )
{
    int8_t remainingRequestBytes = requestLength;
    while ( remainingRequestBytes > 0 )
    {
        pio_sm_put_blocking( pio, sm, request[requestLength - remainingRequestBytes] << 24 );
        remainingRequestBytes--;
    }
}

void Controller::getResponse( PIO pio, uint sm, uint8_t *response, uint8_t responseLength )
{
...
double Controller::getScaledAnalogAxis( double axisPos, double *minAxis, double *maxAxis )
{
